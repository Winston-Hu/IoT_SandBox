...
---
## M300_daily_report.py (Notes)

### Overview

`M300_daily_report.py` 是一个**长驻后台的日报生成服务**，用于对 `monitor.py` 产生的 `monitor_log` 进行每日统计分析，并自动通过邮件发送报告。

该程序会：

1. 只读取当前的 `monitor_log` 文件（**不会读取 .1 / .2 轮转备份**）
2. 在指定时间窗口内筛选关注的异常日志行
3. 提取 `mac` 地址与问题类型
4. 按 **mac + 时间顺序排序**
5. 生成 CSV 报表
6. 每天固定时间自动发送日报邮件
7. 若无异常，仅发送纯文字邮件
8. 自带 `report_log` 滚动日志用于记录程序运行状态与异常

---

### 关注的异常事件 (WATCH_TAGS)

日报只统计 `monitor_log` 中包含以下标签的行：

1. `[/SKE_SOLAR/M300_ping]`
2. `[/SKE_SOLAR/slave_1hour/Morriset/]`
3. `[/SKE_SOLAR/SLAVE_BECOME_0]`

这些日志都代表某个 **M300 从设备（MAC 地址）发生了异常行为**，例如：
